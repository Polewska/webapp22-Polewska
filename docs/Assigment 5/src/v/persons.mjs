...
document.getElementById("RetrieveAndListAll")
  ...
  for (const key of Object.keys(Person.instances)) {
    ...
    //add directed Movies 
    const directedListEl = createListFromMap(person.directedMovies, "movieId");
    row.insertCell().appendChild(directedListEl);
    //add playedMovies 
    const actedListEl = createListFromMap(person.playedMovies, "movieId");
    row.insertCell().appendChild(actedListEl);
  }
