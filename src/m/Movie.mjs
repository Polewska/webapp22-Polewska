...
set director(direct) {
  if(!direct){
    //if director has to be deleted, remove the inverse reference, unset the director property
...
addActor(act) {
  ...
  if (actorId && validationResult instanceof NoConstraintViolation) {
    // add the new actor reference and its inverse reference
    this._actors[actorId] = Person.instances[actorId];
    this._actors[actorId].playedMovies[this._movieId] = this;
  } else {
    throw validationResult;
  }
}
...
removeActor(act) {
  ...
  const validationResult = Movie.checkActor(actorId);
  if (validationResult instanceof NoConstraintViolation) {
    // delete the actor reference and its derived inverse reference
    delete this._actors[actorId].playedMovies[this._movieId];
...
Movie.destroy = function (movieId) {
  ...
  if (movie) {
    ...
    // remove inverse referendes from movie.[director|actor]
    if(movie.director){
